...
interface Turf {
  ...
  location: string;
  rating: number;
  price: number; // This is the base/fallback price
}
...
export default function BookingPage() {
  ...
  const fetchTurfInfo = async () => {
    try {
      ...
      console.error("Error fetching turf info:", error.message);
    }
  };
  ...
  const fetchSlots = async () => {
    ...
    try {
      ...
      let query = supabase
        ...
        .eq("sport", sport)
        .not("status", "in", "('cancelled', 'completed')"); // Ignore non-active bookings
      ...
      // 1. Fetch all pricing rules for this turf and sport
      ...
      // 2. Compute best price for each slot
      const priceMap: Record<string, number> = {};

      for (const slot of slotsWithStatus) {
        const applicablePrices = pricingRules
          .filter((rule) => {
            // A rule is valid ONLY if all its defined properties match.
        ...
        // 3. Apply the best price (or the turf's base price)
        if (applicablePrices.length > 0) {
          priceMap[slot.id] = applicablePrices[0].price; // The best rule
        } else {
          priceMap[slot.id] = turfInfo.price; // Fallback to base price
        }
      }
      // --- END PRICE FIX ---
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          date: formattedDate,
          slot: selectedSlots,
          user_id: customUserId || userId, // Use the new/found ID
      ...
      router.push(
        `/whatsapp-confirmation?` +
          new URLSearchParams({
            ...
            date: formattedDate,
            slots: selectedSlots.join(","), // This needs slot *times*, not IDs
  ...
  const handlePersonalDetailsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existing) {
        ...
        handleConfirmBooking(existing.id); // Pass ID directly
      } else {
        ...
        setShowPersonalDetailsModal(false);
        handleConfirmBooking(newUserData.id); // Pass new ID directly
      }
    } catch (error: any) {
  ...
  slots.forEach((slot) => {
    ...
    else groupedSlots.day.push(slot); // Fallback
  });
                      ...
                      <div key={slot.id} className="relative group">
                        <Button
                          ...
                          className={cn(
                            ...
                            !isSelected && !slot.isBooked && !isPast &&
          ...
          <form onSubmit={handlePersonalDetailsSubmit} className="space-y-6">
            ...
            <div className="flex gap-3 pt-4">
              ...
              <Button type="submit" disabled={isSubmitting} className="flex-1 bg-primary text-white">
                {isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Complete Booking"}
              </Button>
            </div>
          </form>
        </DialogContent>
