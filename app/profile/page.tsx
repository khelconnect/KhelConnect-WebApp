...
// --- FORCE DYNAMIC (Fixes caching issues in production) ---
...
// --- INNER CONTENT COMPONENT (Logic moved here) ---
function ProfileContent() {
    ...
    if (returnBookingId && bookings.length > 0) {
      ...
      if (targetBooking) {
        ...
        // --- SCENARIO B: Failure ---
          ...
          console.log("Payment Failed via URL/DB");
          setPaymentStatusBooking({ ...targetBooking, payment_status: 'failed' });
          router.replace("/profile");
        } 
        
        // --- SCENARIO C: Unknown/Pending -> Start Polling ---
        else {
          console.log("Status unclear, starting poll...");
          ...
          const pollInterval = setInterval(async () => {
              ...
              if (fresh && (fresh.payment_status === "paid" || fresh.status === "confirmed")) {
                  ...
                  router.replace("/profile");
              } else if (attempts >= 10) { // 20s Timeout
                  clearInterval(pollInterval);
                  setIsVerifyingPayment(false);
                  // Only show failure if we timed out
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
        ...
        if (desktopSliderRef.current) { const width = desktopSliderRef.current.clientWidth; desktopSliderRef.current.scrollTo({ left: width * nextIndex, behavior: 'smooth' }); }
        if (mobileSliderRef.current) { const width = mobileSliderRef.current.clientWidth; mobileSliderRef.current.scrollTo({ left: width * nextIndex, behavior: 'smooth' }); }
    }, 5000);
    return () => clearInterval(interval);
    ...
    <main className="container mx-auto px-4 py-8 max-w-6xl pb-24 md:pb-8">
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-8 items-start">
        ...
        <div className="w-full md:w-2/3 flex flex-col gap-4">
            ...
            <div className="hidden md:block flex-1 min-h-[140px] relative overflow-hidden rounded-2xl border border-border bg-card/50">
               <div ref={desktopSliderRef} className="flex overflow-x-auto snap-x snap-mandatory scrollbar-hide h-full w-full" onScroll={handleScroll}>
                  {slides.map((slide, idx) => (
                    <div key={idx} className="w-full flex-shrink-0 snap-center p-6 flex flex-col justify-center">
                        ...
                        {slide.type === 'booking' && (<div className="flex items-center gap-4"><div className="h-12 w-12 rounded-full bg-green-100 flex items-center justify-center"><Calendar className="h-6 w-6 text-green-600"/></div><div><p className="font-medium text-lg">Match at {(slide.data as BookingDetail).turfs.name}</p><p className="text-muted-foreground text-sm">{format(new Date((slide.data as BookingDetail).date), "PPP")} • {getFormattedTimeRange((slide.data as BookingDetail).slot)}</p></div></div>)}
                        {slide.type === 'payment' && (<div className="flex items-center gap-4 cursor-pointer" onClick={() => document.querySelector('[value="payments"]')?.dispatchEvent(new MouseEvent('click', {bubbles: true}))}><div className="h-12 w-12 rounded-full bg-red-100 flex items-center justify-center"><AlertCircle className="h-6 w-6 text-red-600"/></div><div><p className="font-medium text-lg">Payment Action Needed</p><p className="text-muted-foreground text-sm">{(slide.data as BookingDetail[]).length} pending transaction(s). <span className="underline decoration-red-300">View</span></p></div></div>)}
                        {slide.type === 'empty' && (<div className="flex items-center gap-4"><div className="h-12 w-12 rounded-full bg-secondary flex items-center justify-center"><Star className="h-6 w-6 text-muted-foreground"/></div><div><p className="font-medium text-lg">You're all caught up!</p><p className="text-muted-foreground text-sm">Ready for your next game?</p></div></div>)}
                    </div>
                  ))}
               </div>
      ...
      {slides.length > 0 && slides[0].type !== 'empty' && (
        <div className="md:hidden fixed bottom-4 left-4 right-4 z-50">
           <div ref={mobileSliderRef} className="flex overflow-x-auto snap-x snap-mandatory scrollbar-hide rounded-2xl shadow-xl" onScroll={handleScroll}>
              {slides.map((slide, idx) => (
                <div key={idx} className="w-full flex-shrink-0 snap-center">
                    {slide.type === 'payment' && (<div className="bg-red-600 text-white p-4 flex justify-between items-center cursor-pointer" onClick={() => document.querySelector('[value="payments"]')?.dispatchEvent(new MouseEvent('click', {bubbles: true}))}><div className="flex items-center gap-3"><div className="bg-white/20 p-2 rounded-full"><AlertCircle className="h-5 w-5"/></div><div><p className="font-bold text-sm">Action Required</p><p className="text-xs text-white/80">{(slide.data as BookingDetail[]).length} Pending Payment(s)</p></div></div><Button size="sm" variant="secondary" className="text-red-600 h-8 text-xs">View</Button></div>)}
                    {slide.type === 'booking' && (<div className="bg-white/90 backdrop-blur-xl border border-green-500/20 p-4 flex justify-between items-center"><div className="flex items-center gap-3"><div className="bg-green-100 p-2 rounded-full"><Calendar className="h-5 w-5 text-green-700"/></div><div className="text-left"><p className="font-bold text-sm text-foreground">Next Match</p><p className="text-xs text-muted-foreground">{(slide.data as BookingDetail).turfs.name.substring(0, 15)}...</p></div></div><div className="text-right"><p className="text-xs font-bold text-green-700">{format(new Date((slide.data as BookingDetail).date), "MMM d")}</p><p className="text-[10px] text-muted-foreground">{getFormattedTimeRange((slide.data as BookingDetail).slot).split(" - ")[0]}</p></div></div>)}
                </div>
              ))}
           </div>
      ...
      <Dialog open={!!ticketBooking} onOpenChange={(open) => !open && setTicketBooking(null)}><DialogContent className="max-w-md p-0 overflow-hidden bg-transparent border-none shadow-none sm:max-w-md">{ticketBooking && (<div className="relative w-full drop-shadow-2xl"><div className="bg-background rounded-3xl overflow-hidden relative" style={{maskImage: `radial-gradient(circle at 0 13rem, transparent 1rem, black 1rem), radial-gradient(circle at 100% 13rem, transparent 1rem, black 1rem)`, WebkitMaskImage: `radial-gradient(circle at 0 13rem, transparent 1rem, black 1rem), radial-gradient(circle at 100% 13rem, transparent 1rem, black 1rem)`}}><div className="bg-gradient-to-br from-indigo-600 to-violet-700 p-6 pb-8 text-white"><div className="flex justify-between items-start"><div><Badge className="bg-white/20 text-white hover:bg-white/30 border-none mb-3 backdrop-blur-md">CONFIRMED</Badge><h2 className="text-2xl font-bold">{ticketBooking.turfs.name}</h2><p className="text-white/80 text-sm flex items-center gap-1 mt-1"><MapPin className="h-3 w-3"/> {ticketBooking.turfs.location}</p></div><div className="bg-white/10 p-2.5 rounded-xl backdrop-blur-sm"><Ticket className="h-8 w-8 text-white" /></div></div></div><div className="p-6 pt-8 bg-background"><div className="grid grid-cols-2 gap-y-6 gap-x-4"><div><p className="text-[10px] uppercase font-bold text-muted-foreground">Date</p><p className="text-lg font-semibold">{format(new Date(ticketBooking.date), "EEE, MMM d")}</p></div><div><p className="text-[10px] uppercase font-bold text-muted-foreground">Time</p><p className="text-lg font-semibold">{getFormattedTimeRange(ticketBooking.slot)}</p></div></div><div className="flex gap-3 mt-6"><Button variant="outline" className="flex-1 rounded-xl" onClick={handleDownloadTicket}><Download className="h-4 w-4 mr-2"/> Save</Button><Button className="flex-1 rounded-xl bg-indigo-600 text-white" onClick={handleShareTicket}><Share2 className="h-4 w-4 mr-2"/> Share</Button></div></div></div></div>)}</DialogContent></Dialog>
